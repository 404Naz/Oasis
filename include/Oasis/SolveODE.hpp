 ...
 * This method assumes that the differentiation variable is 't', but provides the solution for any variable
 * without loss of generality. Currently only implemented for first order equations.
...
/**
 * Solves for the particular solution of the differential equation. This assumes that the differentiation variable is
 * 't' and the function is f(t).
 ...
 */
static auto SolveNonHomogenousODE(std::vector<std::unique_ptr<Expression>>& terms,
